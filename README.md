...
任务执行期间可打开右上角“任务与日志”，选择对应任务后查看实时输出。拆书或其他任务失败时，可点击“复制日志”直接粘贴错误信息，或点击“下载日志”保存完整 `.log` 文件；历史日志会保留到手动删除任务记录为止。日志包含任务运行信息、模型返回的错误和 Python 异常栈；常见 API Key、Authorization 和 Token 格式会在写入前脱敏。
...
- **章节级混合检索**：故事情节、逐章章纲和正文会按当前任务检索少量相关设定；固定核心规则、BM25 式中文片段匹配、别名扩展和低置信度栏目回退共同降低漏检
- **自然写作优先**：检索结果只作为事实约束，不要求逐条写进正文；正文完成去 AI 味后再做一致性审查，仅对明确冲突局部修正
...
构建完成后，知识库不只用于全书设计。`story-arcs`、`chapter-outlines` 和 `write` 会按当前舞台、情节单元或章纲进行章节级检索：

1. 固定注入少量世界规则、力量边界和技能条件。
2. 用中文片段相关度与精确词匹配检索本轮事实，并自动扩展资料中可识别的别名。
3. 命中置信度不足时，回退到与人物、势力、地点、物品或能力相关的栏目片段。
4. 将结果标记为“只约束、不强制表达”，避免正文为了覆盖设定而写成百科说明。
5. 正文完成自然生成和去 AI 味后，只审查明确的事实冲突，并做最小范围修正。

可在 `file_system/world_knowledge/aliases.json` 手工补充不容易从资料中识别的别名，例如：
...
每次检索的查询、命中栏目、分数、来源片段、实际注入内容和正文一致性审查结果都会保存到 `file_system/knowledge_retrieval/vol_xx/`，便于检查是否漏检。该目录属于小说工作区产物，不是程序测试数据。
